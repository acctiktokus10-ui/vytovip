...
export default function Home() {
  ...
  useEffect(() => {
    const p = localStorage.getItem('vytovip_pwd')
    if (p) {
      setPassword(p)
      fetch('/api/upload', {
        method: 'POST',
        ...
        else { localStorage.removeItem('vytovip_pwd') }
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      setAuthed(true)
      localStorage.setItem('vytovip_pwd', password)
    } catch (err) {
  ...
  function handleLogout() {
    setAuthed(false)
    localStorage.removeItem('vytovip_pwd')
    setPassword('')
  }
        ...
        <header>
          <div className="inner">
            <div className="hd-left">
              ...
              <div><h1>VytoVIP 👑</h1><p>✨ Quản lý dữ liệu</p></div>
            </div>
          ...
          <div className="card csv-card">
            <div className="card-title">
              ...
              <div>
                ...
                <p>Import CSV Shopee Affiliate — liên hệ admin VytoVIP để được hướng dẫn lấy file CSV</p>
              </div>
            </div>
